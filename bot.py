...
async def on_cb(u:Update,c:ContextTypes.DEFAULT_TYPE):
    ...
    # ── Criação ──
        ...
        if ai<5:
            ...
            else: st["atributos_base"][ATTR_KEYS[5]]=chosen;st["step"]="equip";await _fin_attrs(m,uid)
        else: st["step"]="equip";await q.edit_message_text(f"✅ {ATTR_LABELS[5]}: *{chosen}*",parse_mode="Markdown");await _fin_attrs(m,uid)
